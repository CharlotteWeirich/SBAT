...
function shortcutOkayButtonClicked(){
    ...
    // fill the shortcutList with the entered shortcuts
    shortcutList = [];
    for(i = 0; i < labelSet.length; i++){
        shortcutField = document.getElementById(labelSet[i] + 'Shortcut');
        if (shortcutField.value){
            shortcutList.push(shortcutField.value);
        }
        else{
    ...
    // delete the textareas and labels
    ...
    document.onkeyup = function(e){
        for(i = 0; i < shortcutList.length; i++){
            if (e.which == keyCodeList[shortcutList[i]] || e.keyCode == keyCodeList[shortcutList[i]]){
